...
local function FormatCurrency(Money)
	local MoneyTbl = MoneyValues[string.len(Money)]
	local Str

	if MoneyTbl then
		local Sub = tostring(Money):sub(1, MoneyTbl[2])

		Str = string.format([==[%d%s]==], Sub, MoneyTbl[1])
	end
...
local function DoxedStats(Country)
  local Money, Population = FormatCurrency(CountryData[Country].Economy.Balance.Value), FormatCurrency(CountryData[Country].Population.Value)
  local Manpower, ManpowerMax = FormatCurrency(CountryData[Country].Manpower.Value.X), FormatCurrency(CountryData[Country].Manpower.Value.Z)
  ...
  DoxedCountryBalance:Refresh([[Balance: ]] .. FormatCurrency(GetBalance(Country)))
  DoxedCountryPopulation:Refresh([[Population: ]] .. Population)
  DoxedCountryManpower:Refresh(string.format([==[%s: %s / %s]==], [==[Manpower]==], Manpower, ManpowerMax))
